...
Synergykit.Init = function(tenant, key, options) {
    ...
    Synergykit.key = key
    Synergykit.debug = options.debug || false
    Synergykit.api = apiProtocol + "://" + synergykitBasicApi.replace("%s", Synergykit.tenant)
    Synergykit.socketApi = socketProtocol + "://" + synergykitBasicApi.replace("%s", Synergykit.tenant)
    if(options.api) {
        Synergykit.api = options.api
    }
    if(options.socketApi) {
        Synergykit.socketApi = options.socketApi
    }
